...
upgrade.subscription  = function (req, res, next) {
  ...
  getCustomerByUser(req.session.user).then(function (results) {
    ...
    return Promise.all([
      stripe.customers.listSubscriptions(result.stripe_id), // jshint ignore:line
      // stripe.charges.list({ customer: result.stripe_id })
      stripe.charges.retrieve("ch_4lmu0ocxsMJVjc").then(function (data) {
        console.log(data);
        return {
          data: [data]
        }
    ...
    subscriptions.forEach(function (subscription) {
      if (subscription.plan.id.indexOf('vat') !== -1) {
        subscription.plan.vat = subscription.plan.amount - (subscription.plan.amount / 6);
      }
    });
    ...
    res.render('account/subscription', {
      ...
      request: req,
      subscription: subscriptions[0], //only send the first
      charges: values[1],
