...
module.exports = Observable.extend({
  ...
  validateLogin: function (req, res, next) {
    ...
    userModel.load(username, function (err, user) {
      ...
      var newStyleUser = false;
      try {
        // we're unsure why this is causing an exception, so we're catching and
        // dumping the data if it does.
        if (user.created && user.created.getTime()) {
          newStyleUser = true;
        }
      } catch (e) {
        console.error('failed to get user.created.getTime()', user);
      }
